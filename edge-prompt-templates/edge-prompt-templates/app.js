...
// v2.4.3 hardened init (fix escHTML quote, remove stray True, guards)
(function(){
  console.log('[PTS] app v2.4.3 up');
  ...
  const storage = (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) ? wrapChromeStore(chrome.storage.local) : makeLocalStore('__pts__');
  ...
  function onCopyOrInsert(act){
    try{
      ...
      if(act==='copy'){ navigator.clipboard.writeText(txt).then(()=>toast('已复制')); }
        ...
        try{
          if(typeof chrome === 'undefined' || !chrome.runtime || !chrome.runtime.sendMessage){ toast('当前环境不支持注入'); return; }
          chrome.runtime.sendMessage({ type:'INJECT_TO_CHATGPT', text:txt, send:(act==='send'), tmp:!!t.tmpChat, urls }, (res)=>{
            if(chrome.runtime.lastError){ toast('注入失败：'+chrome.runtime.lastError.message); return; }
            if(res?.ok){
              if(act==='send'){
  ...
  function wire(){
    ...
    $('#openSidePanel')?.addEventListener('click', ()=>{ try{ chrome.runtime.sendMessage({ type:'OPEN_SIDE_PANEL' }, (res)=>{ if(chrome.runtime.lastError){ toast('无法打开侧边栏：'+chrome.runtime.lastError.message); return; } if(!res?.ok){ toast(res?.error||'无法打开侧边栏'); } }); }catch(e){ toast('无法打开侧边栏：'+String(e)); } });
